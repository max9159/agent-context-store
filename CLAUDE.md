...
- **`src/packages/cli/`** (`agent-context-store`) — thin CLI wrapper (`src/index.ts`) that parses `process.argv`, calls core functions, and prints results. Also owns the `agent-config/` directory (bundled agent skill templates for Cursor, Claude, Codex).
